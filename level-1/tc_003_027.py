...
class TC_003_027_DataDriven(MoodleDataDrivenTestCase):
    TEST_CASE = "TC-003-027"
    DATA_FILE = "tc_003_027_data.csv"

    def test_tc_003_027_data_driven(self):
        for row in self.rows():
            with self.subTest(row["test_case"]):
                ...
                self.log_step("Enter answer 3")
                self.set_editor_text_by_data_id(row["answer_2_data_id"], row["answer_2_text"])
